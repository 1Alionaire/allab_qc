...
class QCProcessorApp:
    ...
    def process_files(self):
        ...
        for i, filepath in enumerate(all_files):
            ...
            nob_tem_count_from_report = 0
            nob_tem_count_from_analysis = 0
            # 2. Берем информацию из B3 листа SampleAnalyses
            analyst_info = None
            for row in plm_result_sheet.iter_rows(min_row=6, min_col=2, max_col=10):
                cell_i = str(row[5].value).strip()
                #print(f'method_cell : {str(row[5].value).strip()}')
                if cell_i in ['198.4', '198,4', '198.6', '198,6']:
                    nob_tem_count_from_report += 1
            ...
            if "TEM_Calculation" in wb.sheetnames:
                ...
                if tem_sample_sheet['A6'].value is not None:
                    for row in tem_sample_sheet.iter_rows(min_row=6, min_col=1, max_col=19, values_only=True):
                        #print(f'tem analysis cell : {row[0]}')
                        if row[0] is None or str(row[0]) == 'None' or str(row[0]) == '':
                            break
                        else:
                            nob_tem_count_from_analysis += 1
            
            wrong_sample = 0
            if nob_tem_count_from_analysis < nob_tem_count_from_report:
                wrong_sample = 1
                wrong_projoects += 1
                
            result_json[project_info] = {'TEM amount from report': nob_tem_count_from_report,
                                        'TEM amount from analysis': nob_tem_count_from_analysis,
                                        'wrong': wrong_sample}

            wb.close()

        print(wrong_projoects)
