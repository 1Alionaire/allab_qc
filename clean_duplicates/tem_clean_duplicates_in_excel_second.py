        ...
        try:
            ...
            for file_index, filepath in enumerate(all_files):
                ...
                try:
                    ...
                    if not sheet_exists(wb, "TEM_Calculation"):
                        self.log_message(f" {filepath.name} - Лист TEM_Calculation не найден")
                        continue

                    sample_analysis_ws = wb.Worksheets("TEM_Calculation")
                    report_ws = wb.Worksheets("PLM_TEM_Report")

                    if (sample_analysis_ws.Range("A6").Value is None and str(sample_analysis_ws.Range("A6").Value).strip() == ""
                        and sample_analysis_ws.Range("B6").Value is None and str(sample_analysis_ws.Range("B6").Value).strip() == ""):
                        self.log_message(f"{filepath.name} - No Samples")
                        continue

                    if str(sample_analysis_ws.Range("A6").Value) == 'None' and str(sample_analysis_ws.Range("B6").Value) == 'None':
                        self.log_message(f"{filepath.name} - No Samples")
                        continue

                    report_row = 6
                    total_tem_amount_samples = 0
                    while True:
                        ...
                        if lab_id is None:
                            ...
                            if str(lab_id).strip == '':
                                ...
                                if '-' in str(lab_id).strip():
                                    method = str(report_ws.Range(f"G{report_row}").Value).strip()
                                    if method in ["198.6", "198,6", "198.4", "198,4"]:
                                        total_tem_amount_samples +=1
                        report_row += 1

                    self.log_message(f" total_amount_samples: {total_tem_amount_samples} ")

                    tem_sample_row = 6 + total_tem_amount_samples

                    while True:
                        sample_client_id = sample_analysis_ws.Range(f"A{tem_sample_row}").Value
                        sample_lab_id = sample_analysis_ws.Range(f"B{tem_sample_row}").Value
                        ...
                        if (sample_client_id is not None and str(sample_client_id).strip() != ""
                            ...
                            for col in range(1, 19):
                                sample_analysis_ws.Cells(tem_sample_row, col).Value = ""
...
if __name__ == "__main__":
    ...
    # pyinstaller --onefile --windowed --name="TEM_Delete_all_old_dup_samples" tem_clean_duplicates_in_excel_second.py