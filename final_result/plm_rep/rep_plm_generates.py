    ...
    for file in monthly_pivot_dir.glob("*.json"):
        ...
        for analyst, info in grouped_data.items():
            ...
            for record in info:
                ...
                projects_by_date = {key: value for key, value in raw_data.items() if (value['date'][:-8].strip() == record['date'] and 
                                                                                      value['plm_count'] > 0) and 
                ...
                for project, project_info in projects_by_date.items():
                    ...
                    if project_info['plm_count'] > 0 and len(project_info['plm_analysis']) > 0:
